...
test.describe('ID 413620 – Scheduled LIVE Filing', () => {
  ...
  test('Verify scheduled live filing transmission succeeded', async () => {

    /**
     * Step 2–3: Open newly created 13F deal
     */
    const dealRow = page.getByRole('row', { name: /89798.*live filing/i });
    ...
    /**
     * Step 12–13: Schedule Live Filing modal
     */
    ...
    await expect(page.getByText(/WARNING! You are scheduling a LIVE filing/i)).toBeVisible();
    await page.getByRole('button', { name: 'Schedule LIVE Filing' }).click();
    ...
    /**
     * Step 17: Execution Status validation
     */
    ...
    /**
     * Step 18: Accession Number format validation
     */
    await expect(
      page.getByText(/\d{10}-\d{2}-\d{6}/)
    ).toBeVisible();
    ...
    await Promise.all([
      ...
      page.getByRole('button', { name: 'Create Submission and Download' }).click()
    ]);

    await Promise.all([
      page.waitForEvent('download'),
      page.getByRole('button', { name: 'Download Current Submission' }).click()
    ]);

    await Promise.all([
      page.waitForEvent('download'),
      page.getByRole('button', { name: 'Create Decoded Submission and Download' }).click()
    ]);
  });
