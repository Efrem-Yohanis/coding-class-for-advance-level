...
// Email field (first "Email Address")
const emailInput = secFrameLocator.locator('label.form-label:has-text("Email Address")')
  .first()
  .locator('xpath=following-sibling::input[1]');
await expect(emailInput).toBeVisible();

// 4) (Optional) Assert Save button exists in the iframe
await expect(secFrameLocator.getByRole('button', { name: /^Save$/i })).toBeVisible();
